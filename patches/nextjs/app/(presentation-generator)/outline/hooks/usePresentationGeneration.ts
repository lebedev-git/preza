...
export const usePresentationGeneration = (
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const isPptxVerbatimFlow =
        typeof window !== "undefined" &&
        presentationId &&
        window.sessionStorage.getItem("presenton:pptx-verbatim-id") === presentationId;
      const pptxSlidesForVerbatim = isPptxVerbatimFlow
        ? (() => {
          try {
            return JSON.parse(window.sessionStorage.getItem("presenton:pptx-verbatim-slides") || "[]");
          } catch {
            return [];
          }
        })()
      ...
      const response = isPptxVerbatimFlow
        ? await fetch("/api/v1/ppt/presentation/prepare-verbatim", {
          method: "POST",
          headers: getHeader(),
          body: JSON.stringify({
            ...
            outlines,
            layout,
            pptx_slides: pptxSlidesForVerbatim,
          }),
          cache: "no-cache",
        }).then(async (response) => {
          if (!response.ok) {
            const errorText = await response.text();
            throw new Error(errorText || "Failed to prepare verbatim presentation");
          }
      ...
      if (response) {
        ...
        router.replace(
          isPptxVerbatimFlow
            ? `/presentation?id=${presentationId}&type=standard`
