...
const SupportingDoc = ({ files, onFilesChange, onPptxFilesAdded }: SupportingDocProps) => {
    ...
    const notifyPptxFilesAdded = (newFiles: File[]) => {
        const pptxFiles = newFiles.filter(isPptxFile)
        if (pptxFiles.length > 0) {
            onPptxFilesAdded?.(pptxFiles)
        }
    }
    ...
    const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
        ...
        if (validFiles.length > 0) {
            ...
            notifyPptxFilesAdded(validFiles)
