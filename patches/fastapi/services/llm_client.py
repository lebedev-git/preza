...
class LLMClient:
    ...
    async def _generate_ollama_structured(
        ...
        client: AsyncOpenAI = self._client
        response_schema = ensure_array_schemas_have_items(response_format)
        if strict and depth == 0:
            response_schema = ensure_strict_json_schema(
                response_schema,
                path=(),
        ...
        json_instruction = LLMSystemMessage(
            content=(
        ...
        retry_messages: List[LLMMessage] = [json_instruction, *messages]
        last_content = ""

        for attempt in range(2):
            response = await client.chat.completions.create(
                model=model,
                messages=[message.model_dump() for message in retry_messages],
            ...
            parsed = self._parse_json_object_from_content(content)
                ...
                *messages,
                LLMUserMessage(
                    content=(
                        ...
                        "Return only one valid JSON object conforming to the schema. "
                        f"Previous response:\n{content[:2000]}"
                    )
                ),
            ]
        ...
        print(
            "ollama_structured: failed to parse JSON response "
            f"model={model} content_preview={last_content[:500]!r}"
        )
        return None

    def _parse_json_object_from_content(self, content: str) -> Optional[dict]:
        if not content:
            return None
